...
- Galosphere / Spelunkery : un seul des deux au vote → 2 runs.
...
- `bountiful` `8.0.0-beta.2` : aucune dépendance `required` vers Kambrik sur Modrinth, alors que `MODS.md` §2.2 dit qu'il la faut
  → trancher avec le log de démarrage, corriger `MODS.md`.
...
| Run | `level-name` | Mods |
|---|---|---|
| A | `world-A` | tout, sauf `spelunkery` |
| B | `world-B` | A − `galosphere` + `spelunkery` |
...
4. `locate` : 1 biome Terralith + 1 structure par mod de structure/boss. ID de structure lus dans le jar
